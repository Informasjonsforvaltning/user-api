    ...
    if (!mockserver.isRunning) {
        ...
        mockserver.stubFor(
            get(urlPathMatching("/api/maskinporten/token.*"))
                .willReturn(
                    okJson(
                        """{"access_token":"contract-test-token","token_type":"Bearer","expires_in":3600,"scope":"altinn:accessmanagement/authorizedparties.resourceowner"}""",
                    ),
                ),
        )
...
abstract class WiremockContext {
    companion object {
        ...
        init {
            ...
            try {
                val con = URI("http://localhost:5555/ping").toURL().openConnection() as HttpURLConnection
                con.connect()
